...
| `apps/web/src/domain/selectors/promptSelectors.ts` | `engine bridge` | `promptIdentityFromActivePromptPayload()` now projects public/protocol/legacy prompt target identity into an explicit `PromptIdentityViewModel`, and `PromptViewModel.identity.primaryPlayerId` prefers public identity. `isPromptTargetedToLegacyPlayer()`, `promptPrimaryTargetId()`, and `isPromptPrimaryTarget()` centralize rendered UI ownership and queued-prompt target checks on that identity model instead of direct top-level `PromptViewModel.playerId` comparisons. Rendering still requires a numeric `legacy_player_id` bridge before building the actionable prompt view model. | Actionable prompt view-model construction and local viewer identity must no longer require the numeric legacy bridge before this can move out of `engine bridge`. |
