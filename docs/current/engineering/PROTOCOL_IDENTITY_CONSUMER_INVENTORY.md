...
| `apps/web/src/headless/HeadlessGameClient.ts` | `compat alias` | Headless decisions now send public `player_id` when available, expose `HeadlessDecisionContext.identity.primaryPlayerId` to policies, and write compact decision/view traces with `primary_player_id` plus explicit legacy/public companions. Active and raw prompt routing now use the shared prompt identity helper against latest `view_commit.viewer` public/protocol/viewer/seat identity, with a narrow numeric fallback only for legacy-only prompts. Top-level trace `player_id`, duplicate/retry request ledgers, and outbound legacy aliases still use the numeric player seat value for compatibility. | Duplicate suppression, retry matching, and top-level trace keys must no longer require numeric `player_id` when public identity companions are present. |
