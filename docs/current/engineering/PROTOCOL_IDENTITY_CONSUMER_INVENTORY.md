...
| `apps/web/src/hooks/useGameStream.ts` | `compat alias` | UI decision submission passes `playerId`, `legacyPlayerId`, `publicPlayerId`, `seatId`, and `viewerId` into the decision message. Duplicate-flight keys now prefer public/protocol identity and no longer require a numeric legacy bridge when a public identity companion is present, while numeric ids remain accepted for legacy prompts and UI ownership checks. | UI ownership checks and remaining rendered prompt selection must no longer require a numeric player id before this can move out of `compat alias`. |
