...
- latest platform-managed input-path evidence: `deploy/redis-runtime/local-platform-managed.smoke.json` passed the same restart+decision proof through `redis_restart_smoke.py --skip-up` with concrete restart and worker health commands. Topology `local-runtime-platform-managed-decision`, prefix `mrn:{runtime-platform-decision-smoke}`, session `sess_2KtlLh6lzf6vW2bVsxMLu5BT`, restart replay `11 -> 12`, duplicate decision `stale/already_resolved`, post-decision replay `26`.
