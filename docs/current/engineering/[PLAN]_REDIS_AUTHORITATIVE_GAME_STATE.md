...
- latest platform-managed input-path evidence: `deploy/redis-runtime/local-platform-managed.smoke.json` passed the same restart+decision proof through `tools/scripts/redis_platform_smoke_from_manifest.py --run --preflight`, which generated `redis_restart_smoke.py --skip-up` with concrete restart and worker health commands from the manifest. Topology `local-runtime-platform-managed-decision`, prefix `mrn:{runtime-platform-decision-smoke}`, session `sess_4-hrRVNYgxOa79bRESsZwtIp`, restart replay `11 -> 12`, duplicate decision `stale/already_resolved`, post-decision replay `26`.
...
1. Translate the passing local compose smoke into the target hosting platform's native process manifest, then rerun `tools/scripts/redis_platform_smoke_from_manifest.py --manifest <filled-manifest> --run` so the generated `redis_restart_smoke.py` command uses the platform's `--skip-up`, `--restart-command`, `--worker-health-command`, `--expected-redis-hash-tag`, and `--decision-smoke` values.
