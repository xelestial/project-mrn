...
- Module-runner turn movement now enters the same queued movement boundary through `DiceRollModule`: the movement source schedules `apply_move -> resolve_arrival`, then the resulting turn-completion envelope is attached to the active `TurnFrame`'s `TurnEndSnapshotModule.payload.turn_completion`. The turn-owned module emits the turn-end snapshot and advances the turn cursor only after queued actions finish. No `SequenceFrame` turn-completion adapter is valid. The external visual contract remains `dice_roll -> player_move -> landing_resolved -> turn_end_snapshot`.
...
- Decision-bearing effects are starting to become actions too. `request_purchase_tile` now owns purchase precheck and decision only; if the decision bridge raises a prompt boundary, the action is put back at the front of `pending_actions` so replay after Redis recovery resumes the same purchase request rather than skipping or duplicating it. On an affirmative decision it queues `resolve_purchase_tile`, and that later action owns the purchase mutation.
...
- Rent calculation now has the matching tile-trait seed: `RentContext` plus rent modifiers compute base/final rent, modifier breakdowns, and normal-rent waiver consumptions before payment. Derived costs that call `_effective_rent()` intentionally exclude normal rent waivers.
- Score-token placement now has a tile-trait seed and queued action split. `ScoreTokenPlacementContext` computes token room, player hand-token availability, placement limits, and final placement amount before mutation. First-purchase automatic placement runs through `resolve_score_token_placement`; policy-selected own-tile visit placement runs through `request_score_token_placement -> resolve_score_token_placement`.
- Queued unowned-land arrivals now split into `resolve_arrival -> request_purchase_tile -> resolve_purchase_tile -> resolve_score_token_placement -> resolve_unowned_post_purchase` when the player buys and can place a score token. Skip/fail decisions omit `resolve_purchase_tile`; no-token purchases omit `resolve_score_token_placement`. This prevents a human purchase prompt from being raised inside arrival resolution and gives purchase mutation, token placement, adjacent-buy/same-tile/weather post-processing each checkpointable action.
- Queued rent landings now split rent payment from follow-up land effects as `resolve_arrival -> resolve_rent_payment -> resolve_landing_post_effects`. `ArrivalTileModule` only discovers the payable landing, `RentPaymentModule` owns cash/bankruptcy mutation, and `LandingPostEffectsModule` owns adjacent-buy/same-tile follow-up recovery.
...
- latest platform-managed input-path evidence: `deploy/redis-runtime/local-platform-managed.smoke.json` passed the same restart+decision proof through `tools/scripts/redis_platform_smoke_from_manifest.py --run --preflight --evidence-output /tmp/mrn-platform-smoke-evidence.json`, which generated `redis_restart_smoke.py --skip-up` with concrete restart and worker health commands from the manifest and stored the final smoke JSON as a rollout artifact. Topology `local-runtime-platform-managed-decision`, prefix `mrn:{runtime-platform-decision-smoke}`, session `sess_eJiWJunqrTrmwQ62DhzAYwsf`, restart replay `11 -> 12`, duplicate decision `stale/already_resolved`, post-decision replay `26`.
...
1. Translate the passing local compose smoke into the target hosting platform's native process manifest, then rerun `tools/scripts/redis_platform_smoke_from_manifest.py --manifest <filled-manifest> --run --evidence-output <artifact.json>` so the generated `redis_restart_smoke.py` command uses the platform's `--skip-up`, `--restart-command`, `--worker-health-command`, `--expected-redis-hash-tag`, and `--decision-smoke` values and preserves the evidence JSON.
