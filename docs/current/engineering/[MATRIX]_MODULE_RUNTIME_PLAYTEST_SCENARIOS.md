...
| MRN-MOD-005 | 운수 추가 이동/도착 | 운수 결과가 추가 주사위, 이동, 도착을 생성 | `FortuneResolveModule -> MapMoveModule -> ArrivalTileModule` | 추가 액션은 `ActionSequenceFrame`으로 저장되며 새 턴 checkpoint를 만들지 않고, action payload는 해당 native module과 일치해야 함 | stream runtime projection은 active module을 fortune/move/arrival 순서로 표시 | 운수 추가 행동은 현재 시퀀스의 후속 모듈이며 턴 스케줄러를 다시 호출하지 않음 | `GPT/test_runtime_sequence_modules.py`, `GPT/test_runtime_sequence_handlers.py`, `GPT/test_runtime_effect_inventory.py`, `apps/server/tests/test_runtime_semantic_guard.py`, `npm run e2e:module-runtime` |
