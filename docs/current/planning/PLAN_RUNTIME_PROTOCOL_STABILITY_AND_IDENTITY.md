...
- Pending prompt reads, decision submission, prompt delivery marking, prompt expiration, and external decision-result recording accept `public_request_id` as a protocol alias at the `PromptService` boundary, then normalize back to the legacy request key before lifecycle writes, decision storage, and command append. Active prompt/read and submitted decision paths now consult service/store alias indexes before scan fallbacks. This is a storage-boundary lookup migration step, not a canonical storage-key migration.
...
- [ ] Replace canonical prompt storage/resume keys with opaque IDs. Current Phase 0 intentionally keeps legacy semantic prompt request IDs and numeric `prompt_instance_id` as compatibility keys for prompt storage, lifecycle lookup, command replay, and checkpoint compatibility. Runtime prompt sequence seeding and server-side batch continuation no longer parse legacy request-id suffixes for prompt instance or batch identity recovery. Public request alias lookup now uses bounded service/store alias indexes as the first storage-boundary migration step.
