...
- `tests/test_external_ai_full_stack_smoke_script.py` verifies that the external-AI full-stack smoke adapter
  ...
  callback body, omits worker-request top-level `player_id` when public/protocol primary identity is available,
  and retains numeric worker `player_id` only as a compatibility alias for legacy-only prompt input. The
  callback body still sends public/protocol top-level `player_id` until the server callback boundary moves. It
  also verifies that malformed numeric public/protocol primary fields do not leak back into worker `player_id`
  payloads when a public companion exists, and that operator-facing pending-prompt summaries expose
  `primary_player_id`, `primary_player_id_source`, and explicit identity companions next to the raw legacy alias.
