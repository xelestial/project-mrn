...
| Prompt public identity adapter | `apps/server/tests/test_prompt_service.py::test_create_prompt_attaches_opaque_request_identity`, `test_submit_decision_accepts_public_request_id_alias`, `test_decision_command_carries_prompt_player_identity_fields`, `test_simultaneous_batch_collector_response_carries_prompt_player_identity_fields`, `test_simultaneous_batch_timeout_fallback_uses_collector`, `apps/server/tests/test_redis_realtime_services.py::test_prompt_service_accepts_public_request_id_with_redis_prompt_store`, and `apps/server/tests/test_sessions_api.py::test_external_ai_decision_callback_accepts_public_player_and_request_identity` prove public prompt IDs are exposed and accepted at the protocol boundary while decision command materialization preserves public player/seat/viewer companions alongside the legacy numeric routing id. Internal prompt storage still resolves to the legacy request key. |
