...
resolving the numeric value only as the remaining display selector/engine bridge. Derived player,
marker-order, and active-character local highlighting now consume that public/protocol/viewer/seat
identity directly before numeric fallback, and match-table player cards now consume that selector-owned
local flag instead of recomputing it from `effectivePlayerId`. The web prompt selector,
...
legacy compatibility alias while WebSocket outbound decisions submit primary-only
...
Server active prompt view-state now omits top-level `player_id` for
public/protocol active prompts, publishes `primary_player_id` plus source and
public/seat/viewer companions, and keeps `legacy_player_id` as the explicit
numeric bridge. `player_id_alias_role` is emitted only for legacy-only numeric
active prompts.
...
- [ ] Migrate protocol `player_id` payload fields from numeric legacy aliases to string IDs. Current completed state adds string public player, seat, viewer, event, and request identifiers while intentionally keeping numeric `player_id` aliases for compatibility. `SessionService.resolve_protocol_player_id()` now provides the first server-side migration adapter by resolving public string identity fields, including a public string supplied as top-level `player_id`, explicit `primary_player_id` plus `primary_player_id_source`, and legacy numeric aliases back to the internal numeric seat id at one boundary. `SessionService.protocol_identity_fields()` now centralizes additive public identity enrichment and primary identity metadata for downstream payloads. The WebSocket decision route and the external-AI HTTP decision callback accept those string/primary identity fields, normalize them through `SessionService`, then call `PromptService` with the internal numeric `player_id`/`legacy_player_id` bridge required by the remaining prompt/runtime boundary while preserving explicit `primary_player_id` companions for service-level contradiction checks; the reference external-AI worker `/decide` input also accepts explicit `primary_player_id` plus source, `public_player_id`, `seat_id`, or `viewer_id` without top-level `player_id`; external-AI pending prompt reads now return legacy/public request identity plus public player, seat, and viewer companions for worker callbacks; `external_ai_full_stack_smoke.py` omits top-level `player_id` from worker `/decide` requests and server callback bodies when public/protocol primary identity or companion identity is present, sends primary/source plus companions instead, and retains numeric worker/callback `player_id` only for legacy-only prompt input; prompt boundary payloads, `decision_requested` events, `decision_ack` payloads, normal decision commands, simultaneous batch collector responses, batch completion `responses_by_public_player_id` payloads, and timeout fallback responses now carry the public identity fields alongside numeric compatibility aliases. Server active prompt view-state now omits top-level `player_id` for public/protocol active prompts, publishes `primary_player_id` plus source and public/seat/viewer companions, and keeps `legacy_player_id` as the explicit numeric bridge; numeric top-level `player_id` plus `player_id_alias_role` remains only for legacy-only active prompts. Private prompt, `decision_ack`, private decision-event, and embedded active-prompt visibility projection now targets explicit public player, seat, viewer, and primary public/protocol identities before falling back to numeric `player_id` or explicit `legacy_player_id`, so public-only private payloads remain visible to the authorized viewer during migration. `decision_ack` producers now omit top-level `player_id` when `public_player_id` is available, publish `primary_player_id`/source plus public/seat/viewer companions, and preserve the numeric seat only as explicit `legacy_player_id`; numeric-only ACKs remain labeled compatibility aliases. Session bootstrap `session_start.players`, `session_start.snapshot.players`, `session_start.snapshot.board.marker_owner_player_id`, and `session_start.snapshot.board.tiles[].pawn_player_ids` now also carry public plus legacy-prefixed identity companions before runtime fanout begins. Runtime fanout events with direct `player_id` now include the same public identity fields, fanout events with `acting_player_id` now include actor-prefixed public and legacy identity fields, top-level `*_player_id` related-player fields now include prefixed public and legacy identity fields, and top-level `*_player_ids` lists plus `winner_ids` now include public and legacy identity companion lists. Fanout `snapshot.players`, `snapshot.board.marker_owner_player_id`, `snapshot.board.tiles[].owner_player_id`, and `snapshot.board.tiles[].pawn_player_ids` now also get public identity companions and prefixed/list legacy companions while retaining numeric display aliases. Prompt batch continuation maps and resume-token maps now include public-player, seat, and viewer companion fields while retaining numeric routing maps as compatibility aliases. Frontend prompt selection now treats a batch continuation as complete when a public-player, seat, or viewer companion missing/resume pair is complete, so the UI no longer has to see numeric continuation maps to show a batch prompt. Batch-complete resume materialization can now consume public response maps as input and build the numeric engine bridge internally. Prompt sequence seeding and server-side batch continuation no longer parse legacy request-id suffixes for prompt instance or batch identity recovery. Canonical prompt storage/resume keys now use opaque public request IDs while legacy semantic request IDs remain alias/debug inputs. Frontend/headless WebSocket decision construction and the rendered React UI prompt submission path now emit primary-only public/protocol decision identity without top-level `player_id`, carry `legacy_player_id` plus public player/seat/viewer companions, and still keep numeric-only decision payloads unchanged as labeled legacy aliases. The React local viewer state now uses `LocalViewerIdentity` instead of storing a raw numeric local player id, so `view_commit.viewer` public/protocol companions can become primary while the numeric value is resolved only for remaining selector/engine bridge consumers. Frontend board display selectors now consume marker owner, tile owner, pawn-list, and last-move legacy companions before raw board `*_player_id` aliases, so public/protocol board snapshot ids do not erase marker, ownership, pawn, or movement display. Remaining work is eventual numeric alias removal after compatibility gates. The engine's numeric actor-index maps remain internal engine state, not public protocol identity.
...
- Runtime contract schemas for WebSocket outbound decisions, WebSocket inbound prompts, and external-AI
  ...
  WebSocket `outbound.decision.public_identity.json` still freezes the public
  top-level decision path directly: top-level `player_id`, `primary_player_id`,
  and `public_player_id` are the same public string identity, while
  `legacy_player_id` remains the explicit numeric bridge. The WebSocket
  ...
  payloads; `inbound.prompt.public_identity.json` remains public top-level
  prompt compatibility evidence while its nested active view-state freezes
  primary-only active prompt identity, and `inbound.prompt.primary_identity.json`
  freezes that primary-only public prompt shape without replacing the public
  top-level prompt example. Server active prompt view-state now omits top-level
  `player_id` for public/protocol identity and keeps numeric `legacy_player_id`
  as the bridge instead of forcing the top-level field back to a numeric alias.
  The frozen `trick_to_use` prompt example
  ...
  the primary-only public wire conversion to WebSocket `prompt` messages and paired
  ...
  shape when public identity companions are present. The ACK schema also accepts explicit
  ...
  identity example. `build_decision_ack_payload()` now omits top-level
  `player_id` when `public_player_id` is available and preserves the numeric
  bridge only as explicit `legacy_player_id`; numeric-only ACKs remain labeled compatibility
  aliases. The frozen external-AI `decision_requested` event example now also omits top-level
  `player_id`, carries public identity through `primary_player_id`/`public_player_id`, and
  preserves the numeric seat only as `legacy_player_id`, so the runtime contract example
  matches the prompt publishing wire shape.
...
- `apps/server/tests/test_stream_api.py::test_seat_decision_accepts_public_player_identity_with_ack`
  ...
  authenticated internal seat id, and returns a primary-only public ACK payload with `legacy_player_id` as
  the explicit numeric bridge. The ack payload also runs through `assert_no_public_identity_numeric_leaks()`.
...
- `apps/server/tests/test_stream_api.py::test_seat_decision_forwards_primary_player_identity_to_prompt_service`
  and
  `apps/server/tests/test_sessions_api.py::test_external_ai_decision_callback_forwards_primary_player_identity_to_prompt_service`
  verify that those route adapters preserve explicit primary identity companions after resolving the
  internal numeric seat bridge, so `PromptService` remains the final prompt-vs-decision identity
  acceptance boundary.
- `apps/server/tests/test_prompt_service.py::test_submit_decision_accepts_cross_source_primary_identity_when_legacy_bridge_matches`
  and
  `apps/server/tests/test_prompt_service.py::test_submit_decision_rejects_same_source_primary_identity_mismatch`
  verify that `PromptService` accepts a public primary companion submitted against a legacy-only
  prompt when the numeric bridge matches, while still rejecting direct same-source primary identity
  contradictions.
...
- `apps/server/tests/test_prompt_service.py::test_submit_decision_accepts_cross_source_primary_identity_when_legacy_bridge_matches`
  verifies that `PromptService` does not treat source difference alone as a contradiction when the route
  has already resolved the decision to the same explicit `legacy_player_id` bridge.
...
- `apps/web/src/domain/stream/decisionProtocol.spec.ts` and
  ...
  send public/protocol decision identity through explicit `primary_player_id`, `primary_player_id_source`,
  `legacy_player_id`, `public_player_id`, `seat_id`, `viewer_id`, and `public_prompt_instance_id`
  companions without top-level `player_id`, while preserving the existing numeric-only decision path,
  marking top-level numeric `player_id` as a legacy compatibility alias, submitting explicit
  public/protocol primary identity instead of a numeric top-level active-prompt alias when available,
  and keeping the numeric prompt lifecycle alias. `decisionProtocol.spec.ts` also locks the malformed-boundary case where a numeric
...
- `apps/web/src/hooks/useGameStream.spec.ts` verifies that rendered UI decision submission can use a public
  protocol identity without top-level `player_id`, submits explicit public/protocol primary identity
  instead of a numeric active-prompt alias, and builds duplicate-flight keys from explicit prompt primary
  or public identity without requiring a numeric legacy bridge; numeric-only prompt decisions remain
  supported as the legacy fallback.
...
- `apps/web/src/domain/selectors/streamSelectors.spec.ts` verifies that `selectCoreActionFeed()` and
  ...
  display companions when top-level `player_id` is a public/protocol string, and that related-player
  event detail summaries render from prefixed legacy/seat companions when raw `*_player_id` fields carry
  public/protocol strings. It also verifies turn-history participant ids and local relevance consume
...
- `apps/server/tests/test_view_state_prompt_selector.py::ViewStatePromptSelectorTests::test_build_prompt_view_state_emits_primary_only_public_identity_and_legacy_bridge`
  verifies that public/protocol active prompt view-state carries
  `primary_player_id`, source, public/seat/viewer companions, and
  `legacy_player_id` without top-level `player_id`.
...
- `apps/server/tests/test_sessions_api.py::test_external_ai_decision_callback_accepts_public_player_identity`
  ...
  to the internal seat id, and returns a primary-only public ACK payload with `legacy_player_id` as the bridge.
...
- `tests/test_external_ai_full_stack_smoke_script.py` verifies that the external-AI full-stack smoke adapter
  ...
  callback body, omits top-level `player_id` from both payloads when public/protocol primary identity is
  available, and retains numeric worker/callback `player_id` only as a compatibility alias for legacy-only
  prompt input. It also verifies that malformed numeric public/protocol primary fields do not leak back into
  worker/callback `player_id` payloads when a public companion exists, and that operator-facing pending-prompt
  summaries expose `primary_player_id`, `primary_player_id_source`, and explicit identity companions next to the
  raw legacy alias.
...
- `apps/server/tests/test_stream_service.py`, `apps/server/tests/test_visibility_projection.py`, and `apps/server/tests/test_stream_api.py::test_spectator_does_not_receive_prompt_or_decision_ack_for_seat` verify private prompt and `decision_ack` projection boundaries. `test_visibility_projection.py` now also verifies public-only prompt, `decision_ack`, and private decision-event target routing without a numeric `legacy_player_id` bridge, plus embedded public-only active-prompt preservation for the authorized viewer.
