...
- Module decision commands now carry explicit `prompt_instance_id`, and runtime resume sequence matching consumes that field before falling back to legacy request-id parsing. Batch-complete producer payloads now include ordered `expected_public_player_ids`, and resume materialization accepts public-only `responses_by_public_player_id` input by resolving public player IDs through `SessionService` and materializing the numeric bridge map for the engine. This removes one semantic `request_id` dependency, but it does not yet remove batch-id parsing or the engine's numeric batch response application contract.
