...
class RuntimeContractExampleTests(unittest.TestCase):
    ...
    def test_ws_identity_schemas_require_legacy_companion_for_numeric_player_alias(self) -> None:
        root = _project_root() / "packages" / "runtime-contracts" / "ws"
        schemas = root / "schemas"
        ...
        with self.assertRaises(AssertionError):
            _validate_subset(
                {
                    "type": "decision",
                    "request_id": "req_numeric_alias_without_legacy",
                    "player_id": 1,
                    "player_id_alias_role": "legacy_compatibility_alias",
                    "primary_player_id": "player_public_1",
                    "primary_player_id_source": "public",
                ...
                outbound_decision_schema,
                path="$<outbound.decision.numeric_alias_without_legacy>",
            )
            ...
            _validate_subset(
                {
                    "type": "prompt",
                    "seq": 4,
                    "session_id": "sess_numeric_alias_without_legacy",
                    "payload": {
                        "request_id": "req_numeric_alias_without_legacy",
                        "request_type": "movement",
                        "player_id": 1,
                ...
                inbound_prompt_schema,
                path="$<inbound.prompt.numeric_alias_without_legacy>",
            )
    ...
    def test_ws_identity_schemas_reject_string_legacy_primary_identity(self) -> None:
        root = _project_root() / "packages" / "runtime-contracts" / "ws"
        schemas = root / "schemas"
        ...
        with self.assertRaises(AssertionError):
            _validate_subset(
                {
                    "type": "decision",
                    "request_id": "req_string_legacy_primary",
                    "player_id": 1,
                    "player_id_alias_role": "legacy_compatibility_alias",
                    "primary_player_id": "player_public_1",
                ...
                outbound_decision_schema,
                path="$<outbound.decision.string_legacy_primary>",
            )
            ...
            _validate_subset(
                {
                    "type": "prompt",
                    "seq": 4,
                    "session_id": "sess_string_legacy_primary",
                    "payload": {
                        "request_id": "req_string_legacy_primary",
                        "request_type": "movement",
                        "player_id": 1,
                ...
                inbound_prompt_schema,
                path="$<inbound.prompt.string_legacy_primary>",
            )
        ...
        with self.assertRaises(AssertionError):
            _validate_subset(
                ...
                inbound_decision_ack_schema,
                path="$<inbound.decision_ack.string_legacy_primary>",
            )
    ...
    def test_external_ai_request_schema_requires_legacy_companion_for_numeric_player_alias(self) -> None:
        root = _project_root() / "packages" / "runtime-contracts" / "external-ai"
        schema = _load_json(root / "schemas" / "request.schema.json")
    ...
    def test_external_ai_request_schema_rejects_string_legacy_primary_identity(self) -> None:
        root = _project_root() / "packages" / "runtime-contracts" / "external-ai"
        schema = _load_json(root / "schemas" / "request.schema.json")
        ...
        with self.assertRaises(AssertionError):
            _validate_subset(
                ...
                schema,
                path="$<external-ai.request.string_legacy_primary>",
            )
