...
class SessionServiceTests(unittest.TestCase):
    ...
    def test_resolve_protocol_player_id_accepts_public_identity_fields(self) -> None:
        ...
        self.assertEqual(
            self.service.resolve_protocol_player_id(
                session.session_id,
                player_id=join_1["public_player_id"],
                legacy_player_id=1,
                public_player_id=join_1["public_player_id"],
                seat_id=join_1["seat_id"],
                viewer_id=join_1["viewer_id"],
            ),
            1,
        )
    ...
    def test_resolve_protocol_player_id_rejects_conflicting_identity_fields(self) -> None:
        ...
        self.assertIsNone(
            self.service.resolve_protocol_player_id(
                session.session_id,
                player_id=ai_seat["public_player_id"],
                legacy_player_id=1,
            )
        )
        self.assertIsNone(
    ...
    def test_protocol_identity_fields_returns_public_identity_for_internal_seat(self) -> None:
        ...
        self.assertEqual(fields["player_id_alias_role"], "legacy_compatibility_alias")
        self.assertEqual(fields["primary_player_id"], seat_2["public_player_id"])
        self.assertEqual(fields["primary_player_id_source"], "public")
