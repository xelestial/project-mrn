  ...
  surface: {
    ...
    markTarget: {
      ...
      candidates: Array<{
        ...
        targetLegacyPlayerId?: number;
        targetPublicPlayerId?: string;
        targetSeatId?: string;
...
function latestBackendViewStateEntry(
  messages: InboundMessage[],
): { index: number; viewState: Record<string, unknown> } | null {
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    if (messages[i].type !== "view_commit") {
      continue;
    }
    const payload = isRecord(messages[i].payload) ? messages[i].payload : null;
    const viewState = isRecord(payload?.["view_state"])
      ? payload["view_state"]
      : null;
    if (viewState) {
...
function isSecondaryChoiceRecord(
  choiceId: string,
...
function choiceDescriptionText(
  item: Record<string, unknown>,
  value: Record<string, unknown> | null,
): string {
...
function targetPlayerIdentityCompanions(item: Record<string, unknown>): {
  targetLegacyPlayerId?: number;
  targetPublicPlayerId?: string;
  targetSeatId?: string;
  targetViewerId?: string;
  ...
  const targetLegacyPlayerId =
    numberOrNull(item["target_legacy_player_id"] ?? item["legacy_player_id"]) ??
    numberOrNull(item["target_player_id"]);
  if (targetLegacyPlayerId !== null) {
    companions.targetLegacyPlayerId = targetLegacyPlayerId;
  }
  const targetPublicPlayerId =
    stringOrEmpty(item["target_public_player_id"]) ||
    stringOrEmpty(item["public_player_id"]);
  if (targetPublicPlayerId) {
    companions.targetPublicPlayerId = targetPublicPlayerId;
  }
  const targetSeatId =
    stringOrEmpty(item["target_seat_id"]) || stringOrEmpty(item["seat_id"]);
  if (targetSeatId) {
    companions.targetSeatId = targetSeatId;
  }
  const targetViewerId =
    stringOrEmpty(item["target_viewer_id"]) || stringOrEmpty(item["viewer_id"]);
  if (targetViewerId) {
    companions.targetViewerId = targetViewerId;
  }
  return companions;
}
...
function promptIdentitySourceOrNull(
  value: unknown,
): PromptIdentitySource | null {
  return value === "public" || value === "protocol" || value === "legacy"
    ? value
    : null;
}
...
function compareOptionalProtocolPlayerId(
  promptValue: unknown,
  targetValue: unknown,
): boolean | null {
...
export function promptIdentityFromActivePromptPayload(
  active: Record<string, unknown>,
): PromptIdentityViewModel | null {
  const protocolPlayerId = protocolPlayerIdOrNull(active["player_id"]);
  const legacyPlayerId =
    numberOrNull(active["legacy_player_id"]) ??
    numberOrNull(active["player_id"]);
...
export function isPromptPrimaryTarget(
  prompt: PromptViewModel | null,
  targetId: ProtocolPlayerId | null,
): boolean {
  return promptPrimaryTargetId(prompt) === targetId;
}

export function isPromptTargetedToLegacyPlayer(
  prompt: PromptViewModel | null,
  legacyPlayerId: number | null,
): boolean {
...
export function isPromptTargetedToIdentity(
  ...
  const publicMatch = compareOptionalIdentity(
  ...
  const protocolMatch = compareOptionalProtocolPlayerId(
    identity.protocolPlayerId,
    target.protocolPlayerId,
  );
  if (protocolMatch !== null) {
    return protocolMatch;
  }
  const viewerMatch = compareOptionalIdentity(
    identity.viewerId,
    target.viewerId,
  );
  if (viewerMatch !== null) {
  ...
  return isPromptTargetedToLegacyPlayer(
    prompt,
    comparableLegacyPlayerId(target.legacyPlayerId),
  );
}
...
function parsePromptEffectContext(
  raw: unknown,
): PromptEffectContextViewModel | null {
  if (!isRecord(raw)) {
    return null;
  }
  const label =
    stringOrEmpty(raw["label"]) || stringOrEmpty(raw["source_name"]);
  ...
  const tone =
    rawTone === "move" || rawTone === "effect" || rawTone === "economy"
      ? rawTone
      : "effect";
  return {
    label: label || detail,
    ...
    tone,
    source:
      stringOrEmpty(raw["source"]) ||
      stringOrEmpty(raw["source_family"]) ||
      "system",
    ...
    resourceDelta: isRecord(raw["resource_delta"])
      ? { ...raw["resource_delta"] }
      : null,
  };
}
...
function parsePromptBehavior(
  ...
  return {
    normalizedRequestType:
      typeof behavior?.["normalized_request_type"] === "string" &&
      behavior["normalized_request_type"].trim()
        ? behavior["normalized_request_type"]
        : requestType,
    singleSurface: behavior?.["single_surface"] === true,
    autoContinue: behavior?.["auto_continue"] === true,
    chainKey:
      typeof behavior?.["chain_key"] === "string" &&
      behavior["chain_key"].trim()
        ? behavior["chain_key"]
        : null,
    chainItemCount:
      typeof behavior?.["chain_item_count"] === "number"
        ? behavior["chain_item_count"]
        : null,
...
function parsePromptContinuation(
  raw: Record<string, unknown>,
): PromptContinuationViewModel {
  const missingPlayerIds = Array.isArray(raw["missing_player_ids"])
    ? raw["missing_player_ids"]
        .map((item) => numberOrNull(item))
        .filter((item): item is number => item !== null)
    : null;
  const rawResumeTokens = isRecord(raw["resume_tokens_by_player_id"])
    ? raw["resume_tokens_by_player_id"]
    : null;
  const resumeTokensByPlayerId = rawResumeTokens
    ? Object.fromEntries(
        Object.entries(rawResumeTokens)
          .map(
            ([playerId, token]) =>
              [String(playerId), stringOrEmpty(token)] as const,
          )
          .filter(([, token]) => token.length > 0),
      )
    : null;
  const missingPublicPlayerIds = stringArrayOrNull(
    raw["missing_public_player_ids"],
  );
  const resumeTokensByPublicPlayerId = stringRecordOrNull(
    raw["resume_tokens_by_public_player_id"],
  );
  const missingSeatIds = stringArrayOrNull(raw["missing_seat_ids"]);
  const resumeTokensBySeatId = stringRecordOrNull(
    raw["resume_tokens_by_seat_id"],
  );
  const missingViewerIds = stringArrayOrNull(raw["missing_viewer_ids"]);
  const resumeTokensByViewerId = stringRecordOrNull(
    raw["resume_tokens_by_viewer_id"],
  );
  return {
    promptInstanceId: numberOrNull(raw["prompt_instance_id"]),
    publicPromptInstanceId:
      stringOrEmpty(raw["public_prompt_instance_id"]) || null,
    promptFingerprint: stringOrEmpty(raw["prompt_fingerprint"]) || null,
    promptFingerprintVersion:
      stringOrEmpty(raw["prompt_fingerprint_version"]) || null,
...
function declaresModuleContinuation(raw: Record<string, unknown>): boolean {
  const runnerKind = stringOrEmpty(
    raw["runner_kind"] ?? raw["runtime_runner_kind"],
  );
  if (runnerKind === "module") {
    return true;
  }
  return REQUIRED_MODULE_CONTINUATION_FIELDS.some(
    (field) => stringOrEmpty(raw[field]).length > 0,
  );
}

function hasCompleteModuleContinuation(raw: Record<string, unknown>): boolean {
  if (
    !REQUIRED_MODULE_CONTINUATION_FIELDS.every(
      (field) => stringOrEmpty(raw[field]).length > 0,
    )
  ) {
    return false;
...
function parsePromptSurface(
  ...
  const movement = isRecord(surface?.["movement"])
    ? surface?.["movement"]
    : null;
  const lapReward = isRecord(surface?.["lap_reward"])
    ? surface?.["lap_reward"]
    : null;
  const burdenExchange = isRecord(surface?.["burden_exchange_batch"])
    ? surface?.["burden_exchange_batch"]
    : null;
  const markTarget = isRecord(surface?.["mark_target"])
    ? surface?.["mark_target"]
    : null;
  const characterPick = isRecord(surface?.["character_pick"])
    ? surface?.["character_pick"]
    : null;
  const handChoice = isRecord(surface?.["hand_choice"])
    ? surface?.["hand_choice"]
    : null;
  const purchaseTile = isRecord(surface?.["purchase_tile"])
    ? surface?.["purchase_tile"]
    : null;
  const trickTileTarget = isRecord(surface?.["trick_tile_target"])
    ? surface?.["trick_tile_target"]
    : null;
  const coinPlacement = isRecord(surface?.["coin_placement"])
    ? surface?.["coin_placement"]
    : null;
  const doctrineRelief = isRecord(surface?.["doctrine_relief"])
    ? surface?.["doctrine_relief"]
    : null;
  const geoBonus = isRecord(surface?.["geo_bonus"])
    ? surface?.["geo_bonus"]
    : null;
  const specificTrickReward = isRecord(surface?.["specific_trick_reward"])
    ? surface?.["specific_trick_reward"]
    : null;
  const pabalDiceMode = isRecord(surface?.["pabal_dice_mode"])
    ? surface?.["pabal_dice_mode"]
    : null;
  const runawayStep = isRecord(surface?.["runaway_step"])
    ? surface?.["runaway_step"]
    : null;
  const activeFlip = isRecord(surface?.["active_flip"])
    ? surface?.["active_flip"]
    : null;
  return {
        ...
        ? {
            ...
            options: Array.isArray(lapReward["options"])
              ? lapReward["options"].flatMap((item) => {
                  ...
                  return [
                    { choiceId, cashUnits, shardUnits, coinUnits, spentPoints },
                  ];
                })
              : [],
          }
        : null,
    burdenExchangeBatch: burdenExchange
      ? {
          burdenCardCount:
            numberOrNull(burdenExchange["burden_card_count"]) ?? 0,
                ...
                return [
                  {
                    ...
                    ...targetPlayerIdentityCompanions(item),
                  },
                ];
              })
            : [],
    ...
    characterPick: characterPick
      ? {
          phase:
            stringOrEmpty(characterPick["phase"]) ||
            (requestType === "draft_card" ? "draft" : "final"),
          draftPhase: numberOrNull(characterPick["draft_phase"]),
          draftPhaseLabel:
            stringOrEmpty(characterPick["draft_phase_label"]) || null,
          choiceCount:
            numberOrNull(characterPick["choice_count"]) ??
            (Array.isArray(characterPick["options"])
              ? characterPick["options"].length
              : 0),
    ...
    purchaseTile: purchaseTile
      ...
      : null,
    trickTileTarget: trickTileTarget
      ? {
          ...
          candidateTiles: Array.isArray(trickTileTarget["candidate_tiles"])
            ? trickTileTarget["candidate_tiles"]
                .map((item) => numberOrNull(item))
                .filter((item): item is number => item !== null)
            : [],
    ...
    geoBonus: geoBonus
      ...
      : null,
    specificTrickReward: specificTrickReward
      ? {
...
export function promptViewModelFromActivePromptPayload(
  active: Record<string, unknown>,
  ...
  if (
    declaresModuleContinuation(active) &&
    !hasCompleteModuleContinuation(active)
  ) {
    return null;
  }
  const publicContext = isRecord(active["public_context"])
    ? active["public_context"]
    : {};
  const choicesRaw = Array.isArray(active["choices"])
    ? active["choices"]
    : active["legal_choices"];
  return {
    requestId,
    requestType,
    ...
    timeoutMs:
      typeof active["timeout_ms"] === "number" ? active["timeout_ms"] : 30000,
...
function selectBackendActivePrompt(
  messages: InboundMessage[],
): PromptViewModel | null {
  ...
  const prompt = isRecord(entry.viewState["prompt"])
    ? entry.viewState["prompt"]
    : null;
...
export function selectLatestDecisionAck(
  messages: InboundMessage[],
  requestId: string,
): DecisionAckViewModel | null {
  if (!requestId.trim()) {
    return null;
  }
  return (
    selectBackendLatestPromptFeedback(messages, requestId) ??
    selectLatestRawDecisionAck(messages, requestId)
  );
}

function selectBackendLatestPromptFeedback(
  messages: InboundMessage[],
  requestId: string,
): DecisionAckViewModel | null {
  const entry = latestBackendViewStateEntry(messages);
  ...
  const prompt = isRecord(entry.viewState["prompt"])
    ? entry.viewState["prompt"]
    : null;
  const feedback = isRecord(prompt?.["last_feedback"])
...
function selectLatestRawDecisionAck(
  messages: InboundMessage[],
  requestId: string,
): DecisionAckViewModel | null {
...
function selectBackendHandTrayCards(
  messages: InboundMessage[],
): HandTrayCardViewModel[] | null {
  const entry = latestBackendViewStateEntry(messages);
  if (!entry) {
    return null;
  }
  const handTray = isRecord(entry.viewState["hand_tray"])
    ? entry.viewState["hand_tray"]
    : null;
  ...
  return cards.flatMap((item, index) => {
    ...
    const deckIndex =
      typeof item["deck_index"] === "number" ? item["deck_index"] : null;
    const title =
      stringOrEmpty(item["title"]) || stringOrEmpty(item["name"]) || "Card";
    return [
      {
        key: stringOrEmpty(item["key"]) || `${deckIndex ?? index}-${title}`,
        title,
        effect:
          stringOrEmpty(item["effect"]) || stringOrEmpty(item["description"]),
        serial: stringOrEmpty(item["serial"]),
        hidden: item["is_hidden"] === true || item["hidden"] === true,
        currentTarget:
          item["is_current_target"] === true || item["currentTarget"] === true,
      },
    ];
  });
}
...
export function selectPromptInteractionState({
  ...
  const latestAck = requestId
    ? selectLatestDecisionAck(messages, requestId)
    : null;
  const secondsLeft =
    expiresAtMs === null
      ? null
  ...
  if (manualFeedbackMessage.trim()) {
    ...
    latestAck?.status === "rejected" ||
    latestAck?.status === "stale"
  ) {
