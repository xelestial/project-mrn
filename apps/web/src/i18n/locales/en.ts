export const enLocale = {
  ...
  stream: {
    ...
    tilePurchased: (tileDisplay: string, cost: unknown) => `Purchased tile ${tileDisplay} / cost ${cost}`,
    ...
    rentPaid: (payer: unknown, owner: unknown, amount: unknown, tileDisplay: string) =>
      `P${payer} -> P${owner} / ${amount} / tile ${tileDisplay}`,
    fortuneDrawn: (cardName: string) => `Fortune drawn / ${cardName}`,
    fortuneResolved: (summary: string) => `Fortune resolved / ${summary}`,
  ...
  prompt: {
    ...
    requestMeta: (requestId: string, playerId: number, timeoutMs: number, secondsLeft: number | null) =>
      `Request ${requestId} / actor P${playerId} / limit ${Math.ceil(timeoutMs / 1000)}s / ${secondsLeft ?? "-"}s left`,
    context: {
    ...
    movement: {
      rollMode: "Roll dice",
      cardMode: "Use dice card",
      cardGuide: (limit: number) => `Choose dice cards to use. You may use up to ${limit}.`,
      rollButton: "Roll dice",
      rollWithCardsButton: (cards: number[]) => `Roll dice (using cards ${cards.join(", ")})`,
      selectCardsFirst: "Choose dice cards first.",
    },
    trick: {
      usePrompt: "[Choose a trick to use]",
      hiddenPrompt: "[Choose the trick to hide this round]",
      handSummary: (count: number, hiddenCount?: number) =>
        `Full hand ${count}${typeof hiddenCount === "number" ? ` / hidden ${hiddenCount}` : ""}`,
      skipTitle: "[Do not use one this time]",
      skipDescription: "[No trick is used this time.]",
    },
    character: {
      draftPrompt: "[Click the character to use this turn]",
      finalPrompt: "[Choose the final character]",
      ability: (name: string) => `[${name} ability]`,
    },
    mark: {
      noneTitle: "[No mark]",
      noneDescription: "[Do not use the mark effect this time.]",
      title: (target: string) => `[${target}]`,
      description: (targetCharacter: string, targetPlayerId: number) => `[Target character / player: ${targetCharacter} / P${targetPlayerId}]`,
    ...
    busy: "Processing... waiting for engine response",
  },
} as const;
