export const koLocale = {
  ...
  promptHelper: {
    ...
    byType: {
      ...
      start_reward: "20포인트 예산 안에서 게임 시작 재화 조합을 선택하세요.",
      draft_card: "드래프트 단계에서 이번 라운드 후보 인물을 가져가세요.",
      final_character: "최종으로 사용할 인물을 고르세요.",
      final_character_choice: "최종으로 사용할 인물을 고르세요.",
      trick_to_use: "지금 타이밍에 쓸 잔꾀를 선택하거나 쓰지 않음을 고르세요.",
      purchase_tile: "도착한 칸의 땅을 살지 결정하세요.",
  ...
  eventLabel: {
    events: {
      ...
      tile_purchased: "땅 사기",
      rent_paid: "렌트 지불",
      landing_resolved: "도착 결과",
      marker_transferred: "징표 이동",
      marker_flip: "카드 뒤집기",
      start_reward_chosen: "초기 보상 선택",
  ...
  stream: {
    ...
    markerFlip: "카드 뒤집기",
    fValueChange: {
      detail: (before: number, delta: number, after: number) => `종료시간 ${before} + (${delta}) = ${after}`,
      label: "종료시간 변경",
    },
    runtimeError: "런타임 오류",
  ...
  theater: {
    ...
    payoffBeat: {
      tile_purchased: "땅 산 결과",
      rent_paid: "렌트 결과",
      fortune_drawn: "운수 공개",
      fortune_resolved: "운수 효과",
      start_reward_chosen: "초기 보상 결과",
  ...
  prompt: {
    ...
    choice: {
      ...
      buyTile: (pos: number | null, cost: number | null) =>
        pos !== null && cost !== null ? `${pos + 1}번 칸 / 비용 ${cost}` : "도착한 칸의 땅을 삽니다.",
