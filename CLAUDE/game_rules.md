...
- `GameRules`
  - 위 룰 객체를 묶고, 기존 legacy config 필드와 상호 동기화
...
## Legacy sync
`GameRules` synchronizes explicit injected rules back into legacy `config.coins/shards/end/board` fields so older code paths remain compatible during the transition.


## 0.7.60 note
`GameRules` now also carries `economy`, `resources`, `dice`, and `special_tiles` sections so modules can consume injected rule values instead of reading legacy config fields directly.
