...
def rules_from_dict(raw: dict[str, Any] | None) -> GameRules:
    ...
    return GameRules(
        ...
        lap_reward=LapRewardRules(
            ...
            cash_point_cost=int(_pick(lap_raw, 'cash_point_cost', default=1)),
            shards_point_cost=int(_pick(lap_raw, 'shards_point_cost', default=2)),
            coins_point_cost=int(_pick(lap_raw, 'coins_point_cost', default=3)),
            cash_pool=int(_pick(lap_raw, 'cash_pool', default=60)),
            shards_pool=int(_pick(lap_raw, 'shards_pool', default=40)),
