## 0.7.61-v3claude (2026-03-27)

### v3_claude AI 프로파일 추가 및 반복 패치 세션

**개요**
`heuristic_v2_v3_claude` 프로파일을 신규 설계하고, 100판 단위 실험을 반복하며 승률을 최적화했다.
최종 승률 **30%** (arena 라인업: aggressive/v3_claude/control/balanced 기준, 단독 1위).

---

#### 진단 — 100판 로그 분석 핵심 발견

- **구매 수 → 승률**: 구매 4~5개 52%, 구매 6개+ 90%. 구매가 승리의 충분조건.
- **종료 조건별 승률**: F_THRESHOLD 40% (최고), ALIVE_THRESHOLD 25%, NINE_TILES 0%.
- **게임 길이별 승률**: ~25턴 27%, 26~35턴 34%, 36~50턴 9% (최저), 51+턴 60%.
- **36~50턴 구간 병목**: 승자 구매 5.3개 vs v3_claude 2.3개. 짐 청산 + 운수 카드로 게임당 약 10냥 손실이 구매 기회를 막음.
- **구조적 상한**: 이 게임은 25%로 수렴하도록 설계된 게임이 아님. 시스템 이해 기반 전략으로 30%+ 달성 가능.
...
**v1 (28%)** — 첫 설계
- PROFILE_WEIGHTS: `expansion 1.1, economy 1.3, disruption 1.4, meta 1.5, combo 1.0, survival 1.5`
- 우선권 인식, 지목 대상 수 계산, 박수/만신 조각 임계, 교리 계열 가산
- 문제: expansion 1.1 너무 낮아 초반 구매 기회 미활용

**v2 (29%)** — expansion 상향 + 인물 선택 개선
- PROFILE_WEIGHTS: `expansion 1.6, economy 1.4, disruption 1.4, meta 1.5, combo 1.3, survival 1.2`
- 중매꾼/건설업자/사기꾼에 구매 기회 연동 가산
- 파발꾼 후반 구매 기회 있을 때 감점
- 랩보상: 초반 코인 투자 허용, 중후반 현금 강화
- 부작용: burden_cleanup 12건 → 40건 (현금 얇아진 상태에서 청산 이벤트)

**v3 (29%)** — 짐 파산 방어 강화
- 짐 보유 시 survival 가중치 목표 1.4 → 1.6 (v3_claude 전용)
- 짐 없을 때 reserve_floor 경감 (latent 0.65→0.30, expected 0.50→0.20)
- 짐 있을 때 reserve_floor 강화 (latent 0.85, expected 0.70)
- F 레이스 이동 보너스 +0.50+0.30×f_value
- 상황 인식 전략 전환: 상대 타일이 나보다 2개+ 많으면 견제형 가산
- 수치 변화 미미 — 구조적 한계

**v4 (30%) ★ — 시스템 이해 기반 패치**
- **박수 폴백 가치 반영**: 조각 5개 이상 + 짐 보유 시 폴백 확정 → survival +0.8+0.15×payout, economy +0.5. 지목 성공/실패 무관하게 짐 청산 + 현금 보장.
- **조각 임계-1 강제 선택**: 박수 조각 4개(임계 바로 아래)일 때 gap=1 → shard_score urgency_bonus=2.0, cash/coin -0.8 억제.
- **어사 우선권 1 가치 반영**: expansion +0.8+0.3×buy_value, survival +0.6. 우선권 1 = 지목 위협 없음 + 첫 이동 = 빈 땅 선점.
- **탐관오리 초기 가산**: 우선권 1 expansion +0.7+0.2×buy_value, survival +0.5. (세금 로직은 이후 수정)
- **상황 인식 전략 전환**: 상대 타일 격차 2+ 시 추노꾼/자객/만신/교리 계열 disruption +0.5+0.15×gap.
- 효과: 어사 1회 → 15회, 탐관오리 0회 → 8회. F_THRESHOLD 게임 승률 향상.

**v5 (22%) — 회귀, 즉시 롤백**
- S칸 현금 회복 이동 보너스 추가 → 구매 기회 이탈 (seed 49: 구매 3.4→1.8)
- S칸 방문 2.0→3.0, F방문 1.0→0.5로 악화
- 원인: `v3_cash_recovery_mode`에서 S칸 +1.2, 랩 경유 +1.5 보너스가 이동 방향을 왜곡

**v5b (30%)** — v5 롤백 + 탐관오리 이동 보너스 안전 완화
- S칸/랩 현금 회복 이동 보너스 완전 제거
- 탐관오리: 내 땅·빈 땅에 착지할 때만 보너스 (상대 타일 렌트 위험 회피)
- 박수 이중 폴백 가능(짐 2장+ + 조각 10개) 시 survival +0.6 추가
- cleanup_lock 해제: 박수 + 조각 5개+ + 짐 있으면 폴백 확정 → 구매 허용

**v6 (29%)** — 탐관오리 이동 보너스 오류 수정 (잘못된 방향)
- 탐관오리 세금 메커니즘 오해 발견: "내가 상대 칸으로 이동" → 반대였음
- 실제 룰: 관원/상민이 이동할 때 자신의 조각 //2 만큼을 탐관오리에게 지불
- 잘못된 이동 보너스 제거 후 재실험 → v5b 대비 1%p 하락

**v7 (30%)** — 탐관오리 세금 계산 수정
- 세금 = 관원/상민 상대 각자의 `shards // 2` (상대 자신의 조각 기준)
- 상대 이동 버프(주사위 추가) 부작용 35% 상쇄 → 순이득 65% 반영: `net_tax = total_tax * 0.65`

**v8 (30%)** — 어사·탐관오리 양면 카드 관계 명확화
- 발견: `card_no=1, pair=탐관오리/어사` — 같은 카드의 양면, 동시 존재 불가
- `state.active_by_card`가 엔진 레벨에서 이미 보장 → AI 코드 별도 처리 불필요
- 탐관오리 주석 및 로직 정확화: 관원/상민 상대의 조각 //2 만큼 탐관오리에게 지급이 정확한 메커니즘
...
| 로직 | 내용 |
|------|------|
| 박수 폴백 | 조각 5개+ + 짐 있음 → survival +0.8+0.15×payout, economy +0.5, cleanup_lock 해제 |
| 박수 임계-1 | 조각 4개(gap=1) → shard urgency_bonus=2.0, cash/coin -0.8 |
| 어사 우선권 | buy_value > 0 → expansion +0.8+0.3×buy_value, survival +0.6 |
| 탐관오리 세금 | 관원/상민 상대 shards//2 합산, ×0.65 (이동 버프 상쇄) → economy +0.3+0.12×net |
| 짐 없을 때 reserve | latent×0.30, expected×0.20 (완화) |
| 짐 있을 때 reserve | latent×0.85, expected×0.70 (강화) |
| 상황 인식 | 상대 타일 격차 2+: 견제형 disruption +0.5+0.15×gap |
| F 레이스 | F칸 착지 +0.50+0.30×f_value, F2 추가 +0.30 |

---

#### 실험 환경

- 시뮬레이터: Basegame v0.7.61
- 라인업: aggressive / v3_claude / control / balanced (4인, arena)
- seed: 42~51, 각 10판 × 10청크 = 100판
- 패치 파일: `ai_policy.py`
