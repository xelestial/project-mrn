...
## heuristic_v2_v3_claude 프로파일 (2026-03-27 추가)

### 설계 철학

게임 시스템에 대한 근본적 이해를 기반으로 한 전략:
1. **박수 폴백 엔진**: 조각 5개만 확보하면 짐 파산 위험이 없다. 지목 성공 여부와 무관하게 짐 1장 제거 + 비용만큼 현금 회수가 보장된다.
2. **어사/탐관오리 우선권 1 가치**: 먼저 드래프트하고 먼저 이동 → 빈 땅 선점 경쟁에서 유리. 지목 위협도 없다.
3. **구매 우선**: 데이터 기반 확인 — 구매 4개+ 52%, 6개+ 90% 승률. 생존만으론 이길 수 없다.

### PROFILE_WEIGHTS

```python
"v3_claude": {
    "expansion": 1.6,   # 구매 공격성 (balanced 수준)
    "economy":   1.4,   # 현금 흐름
    "disruption":1.4,   # 견제
    "meta":      1.5,   # 징표/인물 제어
    "combo":     1.3,   # 시너지
    "survival":  1.2    # 생존 (과도한 보수성 방지)
}
```

### 핵심 로직

#### 인물 선택 (_character_score_breakdown_v2)

**박수**
- 짐 보유 + 조각 5개+ → 폴백 확정: `survival += 0.8 + 0.15 × payout`, `economy += 0.5`
- 짐 2장+ + 조각 10개+ → 이중 폴백 가능: `survival += 0.6` 추가
- 짐 없음 + 상대 짐 있음 → `survival -= 0.5` (완화)
- 짐 없음 + 지목 불가 → `survival -= 1.2`

**어사** (card_no=1, pair=탐관오리)
- 우선권 1 + buy_value > 0: `expansion += 0.8 + 0.3 × buy_value`, `survival += 0.6`
- 무뢰 속성 봉쇄(산적/자객/탐관오리/사기꾼 있을 때): `disruption += 1.8`

**탐관오리** (card_no=1, pair=어사)
- 어사와 같은 카드의 반대 면 — 드래프트에서 동시 존재 불가
- 세금 메커니즘: 관원/상민 속성 플레이어가 이동 시 **자신의** 조각 `//2` 만큼 탐관오리에게 지급
  - 대가로 관원/상민은 주사위 1개 추가 (이건 상대에게 이동 버프)
  - 관원: 어사·파발꾼·아전 / 상민: 객주·중매꾼
- 점수: `expansion += 0.7 + 0.2 × buy_value`, `survival += 0.5`
- 세금 수입(관원/상민 상대의 `shards//2` 합산 × 0.65): `economy += 0.3 + 0.12 × net_tax`

**상황 인식 전략 전환**
- 상대 최대 타일 - 내 타일 ≥ 2: 견제형(추노꾼/자객/만신/교리 계열) `disruption += 0.5 + 0.15 × gap`

#### 랩 보상 (choose_lap_reward)

- **박수/만신 임계-1 강제**: 조각 gap=1(박수:4개, 만신:6개) → `shard urgency_bonus=2.0`, `cash/coin -= 0.8`
- 짐 없을 때 `reserve_floor`: latent×0.30, expected×0.20 (구매 억제 완화)
- 짐 있을 때 `reserve_floor`: latent×0.85, expected×0.70 (파산 방지 강화)

#### 구매 결정 (choose_purchase_tile)

- 박수 + 조각 5개+ + 짐 있음 → `cleanup_lock = False` (폴백 확정으로 구매 허용)
...
- 구매 0~1 비율 35% — 20턴 이하 단판(랜덤) + 짐 청산·운수 카드 현금 손실 (게임당 ~10냥)
- burden_cleanup 파산 16건/100판 — expansion 1.6 상향으로 현금이 얇아진 구조적 문제
- 36~50턴 구간 승률 9% — 이 구간에서 현금 부족으로 구매가 정체됨

### 패치 이력 요약

v1(28%) → v2(29%) → v3(29%) → **v4(30%)** → v5(22%↓롤백) → v5b(30%) → v6(29%) → v7(30%) → **v8(30%)**

핵심 도약: v4에서 시스템 이해 기반(박수 폴백, 어사/탐관오리 우선권) 반영으로 +2%p 달성.
