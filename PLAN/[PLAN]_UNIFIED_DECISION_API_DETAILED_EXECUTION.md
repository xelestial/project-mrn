...
현재 체크포인트 (2026-04-07):
- `promptSelectors.ts`는 canonical `legal_choices`만 소비한다.
- `PromptOverlay.tsx`는 canonical `public_context` 키를 우선 사용한다.
- `selectTurnStage(...)`는 `public_context.tile_index`가 없을 때도 `legal_choices[].value.tile_index`로 focus tile을 복원한다.
- 남은 WS-D 가치는:
  - overlay 외 selector/theater 경계의 canonical request 활용 확대
  - request_type별 프론트 테스트/브라우저 회귀 확대
  - shared contract fixture/schema와 프론트 소비의 완전 정렬

현재 체크포인트 (2026-04-07, external participant seam):
- 엔진 `DecisionPort` -> 서버 canonical request -> participant client routing은 실제 런타임 경로에 연결되어 있다.
- `external_ai` seat는 더 이상 placeholder 개념이 아니라:
  - HTTP worker contract
  - capability/version health preflight
  - expected worker identity validation
  - auth-header based worker verification
  를 갖는 실제 participant seam이다.
- injected/custom sender/healthchecker가 있어도 worker identity validation은 우회되지 않도록 서버 쪽 공통 검증으로 묶였다.
- 남은 WS-B/WS-E 계열 가치는:
  - 더 강한 운영 worker/service 배포 경로
  - auth/identity failure 분류의 추가 세분화
  - longer mixed-seat browser/E2E coverage

현재 체크포인트 (2026-04-07, stronger-worker replacement readiness):
- stronger-worker 교체 준비는 repo 내부 기준으로 닫혔다.
- 현재 서버/worker/runtime는 다음을 같은 seam 위에서 처리한다:
  - `worker_profile`
  - `worker_adapter`
  - `policy_mode`
  - `policy_class`
  ...
  - readiness / bounded-attempt metadata
- built-in stronger adapter(`priority_score_v1`)는 실제 localhost HTTP round-trip과 mixed-seat coverage로 고정되었다.
- 따라서 남은 WS-B/WS-E 가치는 주로 운영 단계다:
  1. 실제 stronger 외부 worker/service endpoint 연결
  2. 그 연결 위에서 human/local-AI/external-AI 플레이테스트 수행
  3. 새 evidence가 있을 때만 UI/locale/runtime 미세 조정 재개
...
## 2026-04-07 Progress Update (typed providers / engine port / canonical frontend consumption)

- Runtime/server:
  - split `_ServerDecisionPolicyBridge` responsibilities into:
...
- Engine:
  ...
  - current direct `choose_*` waves route through `_request_decision(...)`
  - engine-side request metadata now carries canonical fields:
    - `request_type`
    - `player_id`
    - `round_index`
    - `turn_index`
    - `public_context`
    - `fallback_policy`
  - `GameEngine(..., decision_request_factory=...)` is now also injectable so request creation itself is not hardwired to one server-local adapter shape
- Frontend:
  - `promptSelectors.ts` now consumes canonical `legal_choices`
  - `PromptOverlay.tsx` prefers canonical prompt `public_context`
  - `selectTurnStage(...)` can focus prompt scenes from canonical choice payloads
- Result:
  - WS-B/WS-C are no longer merely preparatory; the runtime now actually exercises the engine `DecisionPort` adapter path
  - WS-D has moved from “fallback-heavy compatibility” toward canonical prompt consumption
  - remaining value is now concentrated in:
    1. shared schema/example closure
