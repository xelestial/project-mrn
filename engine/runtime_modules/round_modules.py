...
def install_player_turn_modules(
    frame: FrameState,
    round_index: int,
    ...
    *,
    ...
    """Replace scheduled player turns after TurnSchedulerModule and before round tail."""
    turn_modules = [
        build_player_turn_module(round_index, player_id, ordinal, session_id=session_id)
        for ordinal, player_id in enumerate(player_order)
    ]
    setup_and_tail = [module for module in frame.module_queue if module.module_type != "PlayerTurnModule"]
    insert_at = next(
        (
            index + 1
            for index, module in enumerate(setup_and_tail)
            if module.module_type == "TurnSchedulerModule"
        ),
        len(setup_and_tail),
    )
    frame.module_queue = [*setup_and_tail[:insert_at], *turn_modules, *setup_and_tail[insert_at:]]
