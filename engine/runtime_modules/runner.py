...
class ModuleRunner:
    ...
    def advance_engine(self, engine: Any, state: Any, decision_resume: Any | None = None) -> dict[str, Any]:
        ...
        This runner keeps the existing rule helpers as the source of
        ...
        module handlers without changing backend/frontend contracts.
        """
        ...
        if frame is None:
            self._install_round_frame_from_state(
                ...
                initial=self._is_initial_round(state),
            )
            frame = self._active_round_frame(state)
        if frame is None:
    ...
    @staticmethod
    def _is_initial_round(state: Any) -> bool:
        return (
            getattr(state, "rounds_completed", 0) == 0
            and getattr(state, "turn_index", 0) == 0
            and getattr(state, "current_weather", None) is None
            and not any(getattr(player, "turns_taken", 0) for player in getattr(state, "players", []))
        )

    def _install_round_frame_from_state(
        self,
        engine: Any,
        state: Any,
        *,
    ...
    def _restore_checkpoint_work_to_module_frames(self, engine: Any, state: Any) -> None:
        if getattr(state, "runtime_frame_stack", None):
            return
        current_player_id = self._current_turn_player_id(state)
        if current_player_id is not None:
            materialize = getattr(engine, "_materialize_scheduled_actions", None)
            if callable(materialize):
                materialize(state, phase="turn_start", player_id=current_player_id)
        if not getattr(state, "pending_actions", None):
            return
        checkpoint_frame = FrameState(
            frame_id="checkpoint:work",
            frame_type="sequence",
            owner_player_id=current_player_id,
            parent_frame_id=None,
            module_queue=[],
            status="running",
            created_by_module_id="checkpoint:resume",
        )
        state.runtime_frame_stack.append(checkpoint_frame)
        self._promote_pending_work_to_sequence_frames(
            engine,
            state,
            parent_frame=checkpoint_frame,
            parent_module=None,
        )
        state.runtime_frame_stack = [
            frame
            for frame in state.runtime_frame_stack
            if frame.frame_id != checkpoint_frame.frame_id
            or frame.module_queue
            or frame.status in {"suspended", "failed"}
        ]

    @staticmethod
    def _current_turn_player_id(state: Any) -> int | None:
        order = list(getattr(state, "current_round_order", []) or [])
        if not order:
            return None
        try:
            return int(order[int(getattr(state, "turn_index", 0) or 0) % len(order)])
    ...
    def _sync_active_player_turn_after_module_work(self, state: Any) -> None:
