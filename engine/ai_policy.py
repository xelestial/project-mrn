...
class BasePolicy:
    ...
    def _character_score_breakdown_v2(self, state: GameState, player: PlayerState, character_name: str) -> tuple[float, list[str]]:
        ...
        if is_matchmaker(character_name):
            ...
            if profile == "v3_engine" and cleanup_pressure < 1.6 and liquidity["cash_after_reserve"] >= 0.0:
                expansion += 0.95 + 0.20 * land_race["near_unowned"]
                reasons.append("v3_safe_expansion_window")
            if profile == "v3_engine" and cleanup_pressure < 1.25 and liquidity["cash_after_reserve"] >= 1.0:
        ...
        if is_swindler(character_name):
            ...
            if profile == "v3_engine" and cleanup_pressure < 1.8 and liquidity["cash_after_reserve"] >= -0.5:
                expansion += 0.65
        ...
        if is_eosa(character_name):
            ...
            if profile == "v3_engine" and land_race["early_round"] > 0.0:
        ...
        if is_gakju(character_name):
            ...
            if profile == "v3_engine" and cleanup_pressure < 1.4 and liquidity["cash_after_reserve"] >= 0.5:
    ...
    def choose_trick_to_use(self, state: GameState, player: PlayerState, hand: list[TrickCard]) -> TrickCard | None:
        ...
        for card in hand:
            ...
            if token_profile and card.name in {"재뿌리기", "긴장감 조성"} and player.hand_coins >= 2:
                score += 0.5
            if self._profile_from_mode() == "v3_engine":
...
class HeuristicPolicy(BasePolicy):
    ...
    def choose_trick_to_use(self, state: GameState, player: PlayerState, hand: list[TrickCard]) -> TrickCard | None:
        ...
        for card in hand:
            ...
            if self._profile_from_mode() == "v3_engine" and card.name in {"성물 수집가", "도움 닫기"}:
                details[card.name] = -999.0
                continue
            details[card.name] = round(score, 3)
            if score > best_score:
    ...
    def choose_lap_reward(self, state: GameState, player: PlayerState) -> LapRewardDecision:
        ...
        if mode.startswith("heuristic_v2_") or mode == "heuristic_v3_engine":
