...
def test_game_engine_emits_visual_stream_that_validates() -> None:
    ...
    policy = HeuristicPolicy("heuristic_v3_engine", "heuristic_v3_engine", rng=random.Random(777))
    engine = GameEngine(
        DEFAULT_CONFIG,
        policy,
...
def test_round_boundary_visual_events_carry_active_faces() -> None:
    stream = VisEventStream()
    policy = HeuristicPolicy("heuristic_v3_engine", "heuristic_v3_engine", rng=random.Random(123))
    engine = GameEngine(
        DEFAULT_CONFIG,
        policy,
