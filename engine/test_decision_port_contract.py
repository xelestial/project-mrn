...
class DecisionPortContractTests(unittest.TestCase):
    ...
    def test_apply_character_start_defers_mark_target_to_module_target_judicator(self) -> None:
        port = RecordingDecisionPort()
        engine = GameEngine(DEFAULT_CONFIG, DummyPolicy(), rng=random.Random(0), enable_logging=False, decision_port=port)
        state = GameState.create(DEFAULT_CONFIG)
        ...
        self.assertNotIn("choose_mark_target", [request.decision_name for request in port.requests])

    def test_character_mark_target_resolution_routes_through_decision_port(self) -> None:
        port = RecordingDecisionPort()
        engine = GameEngine(DEFAULT_CONFIG, DummyPolicy(), rng=random.Random(0), enable_logging=False, decision_port=port)
        state = GameState.create(DEFAULT_CONFIG)
        player = state.players[0]
        ...
        with patch.object(engine, "_coerce_mark_target_character", return_value=("선비", False)):
            target, _ = engine._resolve_character_mark_target_decision(state, player)

        self.assertEqual(target, "선비")
