...
def test_v3_engine_no_longer_forces_baksu_before_five_shards_without_window():
    state = _make_state()
    ...
    policy = HeuristicPolicy(character_policy_mode="heuristic_v3_engine")
    choice = policy.choose_final_character(state, player, [1, 2])
    assert choice == "객주"


def test_v3_engine_prefers_shards_for_baksu_before_five_shards():
    state = _make_state()
    player = state.players[0]
    ...
    player.shards = 4
    player.hand_coins = 1
    policy = HeuristicPolicy(character_policy_mode="heuristic_v3_engine", lap_policy_mode="heuristic_v3_engine")
    decision = policy.choose_lap_reward(state, player)
    assert decision.choice == "shards"


def test_v3_engine_allows_safe_low_cost_t3_for_online_baksu():
    state = _make_state()
    player = state.players[0]
    player.current_character = "박수"
    player.cash = 12
...
def test_v3_engine_prefers_coins_after_generic_shard_threshold():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.cash = 14
...
def test_v3_engine_allows_safe_growth_t2_buy_when_not_under_pressure():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.cash = 15
...
def test_v3_engine_prefers_controller_over_precheckpoint_baksu_when_cash_starved():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.cash = 5
...
def test_v3_engine_prefers_shards_over_coins_when_cleanup_is_building():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.cash = 10
...
def test_v3_engine_prefers_cash_over_coins_when_cleanup_and_cash_are_critical():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.cash = 4
...
def test_v3_engine_flips_leader_gakju_when_public_lap_engine_is_live():
    state = _make_state()
    player = state.players[0]
...
def test_v3_engine_records_lap_engine_intent_for_gakju_choice():
    state = _make_state()
    player = state.players[0]
    player.cash = 12
    player.shards = 5
...
def test_v3_engine_avoids_relic_collector_without_immediate_shard_window():
    state = _make_state()
    player = state.players[0]
    player.current_character = "교리 감독관"
    player.position = 20
...
def test_v3_engine_avoids_help_run_without_encounter_window():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.position = 0
...
def test_v3_engine_filters_impossible_fast_mark_guesses_for_baksu():
    state = _make_state()
    player = state.players[0]
    player.current_character = "박수"
...
def test_v3_engine_applies_strong_penalty_to_generic_two_card_land_grab_for_gakju():
    state = _make_state()
    player = state.players[0]
    player.current_character = "객주"
    player.position = 0
