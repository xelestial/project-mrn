...
def test_play_html_renderer() -> list[str]:
    ...
    if "Hide nothing" in html:
        errors.append("play HTML still exposes removed hidden-trick skip wording")
    if "Skip (no trick)" in html:
        errors.append("play HTML still exposes removed trick skip wording")
...
def test_human_policy_prompt_and_response() -> list[str]:
    ...
    try:
        ...
        if "type" in policy.pending_prompt:
            errors.append("pending_prompt should not expose removed key 'type'")
        if "options" in policy.pending_prompt:
            errors.append("pending_prompt should not expose removed key 'options'")
        ...
        if not done.is_set():
            ...
            errors.append("choice_id=dice should yield use_cards=False")
...
def test_human_policy_final_character_returns_name() -> list[str]:
    ...
    try:
        ...
        state = GameState.create(DEFAULT_CONFIG)
        player = state.players[0]
        state.active_by_card[1] = CARD_TO_NAMES[1][0]
        state.active_by_card[2] = CARD_TO_NAMES[2][0]
        ...
        if "type" in prompt or "options" in prompt:
            errors.append(f"final_character prompt still exposes removed mirror keys: {prompt}")
        legal_choices = prompt.get("legal_choices", [])
        expected_labels = [CARD_TO_NAMES[1][0], CARD_TO_NAMES[2][0]]
        if [opt.get("label") for opt in legal_choices] != expected_labels:
        ...
        elif result[0] != CARD_TO_NAMES[2][0]:
            errors.append(f"Expected character name {CARD_TO_NAMES[2][0]!r}, got {result[0]!r}")
    except Exception as e:
        import traceback
        errors.append(f"Exception: {e}\n{traceback.format_exc()}")
    return errors
...
def test_prompt_endpoint_idle(port: int) -> list[str]:
    ...
    try:
        ...
        if "type" in data:
            errors.append("idle /prompt payload should not expose removed key 'type'")
