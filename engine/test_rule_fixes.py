...
from runtime_modules.modifiers import seed_builder_purchase_modifier, seed_character_start_modifiers
...
class RuleFixTests(DraftRuleAssertionMixin, unittest.TestCase):
    ...
    def test_builder_gets_free_landing_purchase_without_spending_shards(self):
        ...
        player.shards = 0
        player.position = first_three_land_block_positions(state)[1]
        seed_builder_purchase_modifier(state, player_id=player.player_id, source_module_id="test:builder")
        result = engine._resolve_landing(state, player)
        self.assertEqual(result["type"], "PURCHASE")
    ...
    def test_scheduled_mark_resolves_before_target_turn_start(self):
        ...
        state.runtime_frame_stack = [
            build_turn_frame(
                1,
                target.player_id,
                parent_module_id="mod:round:1:playerturn:p1",
        ...
        steps = []
        for _ in range(8):
            steps.append(engine.run_next_transition(state))
            event_types = [event.event_type for event in stream.events]
            if "turn_start" in event_types:
                break

        self.assertIn("resolve_mark", [step.get("action_type") for step in steps])
        self.assertEqual(event_types[0], "mark_resolved")
...
class TrickSystemTests(DraftRuleAssertionMixin, unittest.TestCase):
    ...
    def test_round_end_marker_flip_waits_until_turn_completion_after_trick(self):
        ...
        state.runtime_frame_stack = [
            build_turn_frame(
                1,
                player.player_id,
                parent_module_id="mod:round:1:playerturn:p3",
                session_id="test-session",
            )
        ]
        ...
        for _ in range(16):
            ...
            steps.append(step)
            if step.get("action_type") == "apply_move":
                break

        self.assertIn("apply_move", [step.get("action_type") for step in steps])
        self.assertIn("trick_used", [row["event_type"] for row in stream.to_list()])
    ...
    def test_queued_trick_action_resumes_after_effect_without_second_trick_prompt(self):
        ...
        for _ in range(24):
            step = engine.run_next_transition(state)
            steps.append(step)
            if step.get("action_type") == "continue_after_trick_phase":
                resumed = step
                break
        ...
        self.assertEqual(len(stream.by_type("trick_window_closed")), 1)
...
class TrickRuleAuditTests(unittest.TestCase):
    ...
    def test_eosa_suppressed_muroe_skill_is_visible_event(self):
        ...
        actor.current_character = "자객"
        eosa.current_character = "어사"
        seed_character_start_modifiers(state, source_module_id="test")
        state.current_round_order = [actor.player_id]
        state.turn_index = 0
        state.runtime_frame_stack = [
            build_turn_frame(
        ...
        for _ in range(4):
            ...
            if stream.by_type("ability_suppressed"):
                break
