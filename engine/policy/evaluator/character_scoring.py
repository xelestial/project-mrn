...
def evaluate_v2_expansion_rules(
    ...
    *,
    ...
    if character_name == MATCHMAKER:
        ...
        if profile == "v3_engine" and inputs.cleanup_pressure < 1.6 and inputs.cash_after_reserve >= 0.0:
            expansion += 0.95 + 0.20 * inputs.near_unowned
            reasons.append("v3_safe_expansion_window")
        if profile == "v3_engine" and inputs.cleanup_pressure < 1.25 and inputs.cash_after_reserve >= 1.0:
    ...
    if character_name == SWINDLER:
        ...
        if profile == "v3_engine" and inputs.cleanup_pressure < 1.8 and inputs.cash_after_reserve >= -0.5:
            expansion += 0.65
...
def evaluate_v2_tactical_rules(
    ...
    if character_name == EOSA:
        ...
        if inputs.profile == "v3_engine" and inputs.early_round > 0.0:
    ...
    if character_name == INNKEEPER:
        ...
        if inputs.profile == "v3_engine" and inputs.cleanup_pressure < 1.4:
