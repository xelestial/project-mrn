...
def assess_purchase_decision(
    *,
    ...
    token_preferred = token_window_value >= benefit + (1.2 if profile == "v3_engine" else 1.6) and not complete_monopoly and not blocks_enemy
    v3_cleanup_soft_block = False
    safe_growth_token_override = False
    if profile == "v3_engine" and purchase_window is not None:
