...
def choose_trick_to_use_runtime(policy: Any, state: Any, player: Any, hand: list[Any]) -> Any:
    ...
    def score(card: Any) -> float:
        ...
        if policy._profile_from_mode() == "v3_engine" and card.name in {"?ê¹…Ðª ?ì„ì­›åª›Â€", "?ê¾©? ?ãƒªë¦°"}:
            return -999.0
        return value
            ...
            _payload_with_trace(
                ...
                DecisionTrace(
                    ...
                    features={
                        ...
                        "token_opt_profile": policy._profile_from_mode() in {"token_opt", "v3_engine"},
                        "candidates": [],
                    },
                    detector_hits=(
                        build_detector_hit(
...
def choose_active_flip_card_runtime(policy: Any, state: Any, player: Any, flippable_cards: list[int]) -> Any:
    ...
    scored = {}
    reasons = {}
    advanced_marker_mode = policy._is_v2_mode() or policy._profile_from_mode() == "v3_engine"
...
def choose_trick_to_use_runtime(policy: Any, state: Any, player: Any, hand: list[Any]) -> Any:
    ...
    def score(card: Any) -> float:
        ...
        if policy._profile_from_mode() == "v3_engine" and card.name in {"성물 수집가", "도움 닫기"}:
            return -999.0
        return value
