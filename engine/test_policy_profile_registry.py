...
def test_profile_registry_resolves_stable_keys_and_legacy_modes() -> None:
    ...
    assert DEFAULT_PROFILE_REGISTRY.resolve_profile_key("heuristic_v3_engine") == "v3_engine"
    assert DEFAULT_PROFILE_REGISTRY.canonicalize_character_mode("control") == "heuristic_v2_control"
    assert DEFAULT_PROFILE_REGISTRY.canonicalize_lap_mode("v3_engine") == "heuristic_v3_engine"


def test_heuristic_policy_canonicalizes_profile_alias_inputs() -> None:
    policy = HeuristicPolicy(character_policy_mode="control", lap_policy_mode="v3_engine", player_lap_policy_modes={1: "token_opt"})
    ...
    assert policy._profile_from_mode(policy.lap_policy_mode) == "v3_engine"


def test_arena_policy_normalizes_per_player_profile_modes() -> None:
    arena = ArenaPolicy(
        player_character_policy_modes={1: "control", 2: "heuristic_v3_engine"},
        player_lap_policy_modes={1: "token_opt", 2: "v3_engine"},
    )

    assert arena.player_character_policy_modes[1] == "heuristic_v2_control"
    assert arena.player_character_policy_modes[2] == "heuristic_v3_engine"
