...
- Used `ai_decisions.jsonl` analysis to tune Engine decision nodes instead of only reading end-game summaries.
...
- Softened the Engine-only cleanup purchase soft-block thresholds so moderate cleanup pressure no longer suppresses as many otherwise survivable land buys.
...
- Added regression coverage for the newly traced decision families and reran 30-game / 100-game `heuristic_v3_engine` smoke batches without runtime crashes.
...
- Added regression coverage for the new trace substrate and ran a 30-game `heuristic_v3_engine` smoke batch without runtime errors.
...
- Completed the Engine human-play prompt-envelope cleanup so the active runtime now consumes canonical `request_type` / `legal_choices` / `public_context` fields without removed `type` / `options` mirrors or top-level context flattening.
...
- Tightened `heuristic_v3_engine` around cleanup / reserve stress: stronger anti-growth penalties under distress, earlier cash fallback, and stricter non-blocking land-buy rejection when token windows or cleanup risk are better survival lines.
...
- Wired token, lap reward, takeover, force sale, and end condition lookups through injected rules while keeping config mirror fields synchronized.
...
- `GameState` now builds mutable `TileState` objects from metadata so tile index, color, purchase cost, rent, owner, and placed score coins can be inspected from a single source of truth while tile metadata mirror views keep `state.board`, `state.tile_owner`, and `state.tile_coins` access stable.
...
- AI purchase tuning: `v3_engine` now lets safe growth buys override small token-window waits, and logs the override as `safe_growth_beats_token_wait`.
...
## 0.7.61v3 - 2026-04-15
- Engine-only cleanup pass: `random`/`arena` profile aliases now resolve through the default profile registry without breaking runtime debug/trick-choice paths.
- Engine survival cleanup: pure growth hard-blocking now only applies to actual expansion faces, so `객주` lap-engine choices are no longer vetoed as if they were `중매꾼`/`건설업자`/`사기꾼`.
- Engine reserve cleanup: `박수` online/stable shard checkpoints were aligned across survival guard and purchase exceptions (`5` online, `7` very stable).
- test harness cleanup: replay/live/human-play pytest modules now assert on error lists instead of returning them, removing `PytestReturnNotNoneWarning` noise.
- test import cleanup: shared engine/engine path/bootstrap logic now lives in `test_import_bootstrap.py`, with root/package `conftest.py` files delegating to the same helper.
...
- Updated README/ai_policy docs to mark `basegame_current_python_code_v7_61_v3_engine_patch.zip` as the current uploaded baseline and to explain the hard-block semantics explicitly.

- fix: 탐관오리 공납은 대상 조각이 아니라 탐관오리 자신의 조각(//2)을 기준으로 적용하도록 수정했다.
- fix: 어사/탐관오리는 같은 카드이므로 탐관오리 패시브의 불필요한 어사 차단 체크를 제거했다.

- v7.61 v3 Engine winpush2: intended to make Engine recognize Ajeon burst windows on stacked enemy pawns and Gakju lap-engine windows near board end with mobility support.
