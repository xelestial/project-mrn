...
class GameState:
    ...
    rng_state_b64: str = ""
    runtime_runner_kind: str = "module"
    runtime_checkpoint_schema_version: int = 3
    ...
    def _checkpoint_pending_action_payloads(self) -> list[dict[str, Any]]:
        actions = [action.to_payload() for action in self.pending_actions]
        seen = {str(action.get("action_id") or "") for action in actions}
        for frame in self.runtime_frame_stack:
            if frame.status == "completed":
                continue
            for module in frame.module_queue:
                if module.status in {"completed", "skipped", "failed"}:
                    continue
    ...
    def to_checkpoint_payload(self) -> dict:
        checkpoint_pending_actions = self._checkpoint_pending_action_payloads()
        return {
            "schema_version": 1,
            ...
            "pending_actions": checkpoint_pending_actions,
            "pending_actions_from_runtime_modules": (
                not self.pending_actions
                and bool(checkpoint_pending_actions)
                and any(frame.status != "completed" for frame in self.runtime_frame_stack)
            ),
    ...
    def from_checkpoint_payload(cls, config: GameConfig, payload: dict) -> "GameState":
        ...
        if bool(payload.get("pending_actions_from_runtime_modules", False)):
        ...
        state.runtime_checkpoint_schema_version = max(
            3,
            int(payload.get("runtime_checkpoint_schema_version", state.runtime_checkpoint_schema_version) or 3),
        )
