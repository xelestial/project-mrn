...
def test_module_runner_session_builds_explicit_round_frame_before_setup_steps() -> None:
...
def test_module_player_turn_spawns_turn_frame_without_legacy_take_turn(monkeypatch) -> None:
    ...
    for _ in range(8):
        ...
        if setup_result.get("module_type") == "TurnSchedulerModule":
            break
    else:
        raise AssertionError("round setup did not reach TurnSchedulerModule")

    def forbidden_take_turn(*_args, **_kwargs) -> None:
        raise AssertionError("module runner must not call direct _take_turn")
...
def test_suspended_child_sequence_returns_to_next_turn_module() -> None:
    ...
    ModuleRunner()._sync_active_player_turn_after_module_work(state)
