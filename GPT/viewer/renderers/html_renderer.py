...
function renderPlayers(frame) {
  ...
  (frame.players || []).forEach((player) => {
    ...
    card.innerHTML = `<div class="player-name" style="color:${color}"><span>P${pid}</span><span>${player.alive === false ? "OUT" : `tile ${Number(player.position || 0) + 1}`}</span></div><div class="player-character">${player.character || "-"}</div><div class="status-sub">${player.display_name || `Player ${pid}`}</div><div class="stat-row" style="margin-top:6px"><span class="chip">$ ${player.cash ?? "?"}</span><span class="chip">Sh ${player.shards ?? "?"}</span><span class="chip">승점 보유 ${player.hand_score_coins ?? "?"}</span><span class="chip">승점 배치 ${player.placed_score_coins ?? "?"}</span><span class="chip">타일 ${player.owned_tile_count ?? "?"}</span></div><div style="margin-top:6px"><span class="mark mark-${markStatus}">${markStatus}</span></div><div class="status-sub" style="margin-top:6px">Public tricks: ${tricks}${player.hidden_trick_count ? ` (+${player.hidden_trick_count} hidden)` : ""}</div>${player.pending_mark_source ? `<div class="status-sub">Marked by P${player.pending_mark_source}</div>` : ""}${burdens.length ? `<div class="status-sub">Burdens: ${burdens.join(", ")}</div>` : ""}${effects.length ? `<div class="chip-row" style="margin-top:6px">${effects.map((name) => `<span class="chip">${name}</span>`).join("")}</div>` : ""}`; panel.appendChild(card);
  });
}
