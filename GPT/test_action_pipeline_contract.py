...
ENGINE_COMPAT_HELPER_ALLOWED_CALLERS: dict[str, set[str]] = {
    "_advance_player": set(),
...
def test_engine_immediate_movement_compat_helpers_are_limited_to_legacy_callers() -> None:
    source = (GPT_ROOT / "engine.py").read_text(encoding="utf-8")
    tree = ast.parse(source)
    offenders: list[str] = []

    class CompatHelperVisitor(ast.NodeVisitor):
        ...
        def visit_Call(self, node: ast.Call) -> None:  # noqa: N802
            if isinstance(node.func, ast.Attribute):
                ...
                if helper_name in ENGINE_COMPAT_HELPER_ALLOWED_CALLERS:
                    caller = self._function_stack[-1] if self._function_stack else "<module>"
                    allowed_callers = ENGINE_COMPAT_HELPER_ALLOWED_CALLERS[helper_name]
                    if caller not in allowed_callers:
                        offenders.append(f"{helper_name} called by {caller} at line {node.lineno}")
            self.generic_visit(node)

    CompatHelperVisitor().visit(tree)
