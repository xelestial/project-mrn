...
def legacy_runtime_module_for_event(
    state: Any,
    event_type: str,
    phase: str,
    player_id: int | None,
    ...
    *,
    ...
    if event_type in ROUND_EVENT_MODULES:
        ...
        module_id = f"legacy:round:{round_index}:{slug}"
        ...
        frame_id = f"seq:{sequence_kind}:{round_index}:p{owner}:legacy"
        module_id = f"legacy:seq:{sequence_kind}:{round_index}:p{owner}:{slug}"
        frame_type = "sequence"
        module_path = [round_frame, turn_frame, frame_id, module_id]
    else:
        module_type, slug = TURN_EVENT_MODULES.get(event_type, ("LegacyEventModule", _slug(event_type or phase)))
        owner = 0 if actor is None else actor
        round_frame = f"round:{round_index}"
        frame_id = f"turn:{round_index}:p{owner}"
        module_id = f"legacy:turn:{round_index}:p{owner}:{slug}"
        frame_type = "turn"
...
def _idempotency_key(
    *,
    ...
    if request_id:
        return f"legacy:{session_id}:request:{request_id}:{event_type}"
    ...
    return f"legacy:{session_id}:round:{round_index}:turn:{turn_index}:{module_id}:{event_type}:{suffix}"
