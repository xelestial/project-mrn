...
- Fortune decision-action note: the subscription-style fortune card now produces `resolve_fortune_subscription` on the queued fortune path. The action owns empty-block target selection and purchase resolution, so target-selection prompts can be checkpointed instead of nesting inside fortune draw resolution.
