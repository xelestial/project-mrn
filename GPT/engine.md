...
- Queue note: queued `apply_move` actions schedule resumable follow-up actions instead of resolving landing inline; direct compatibility calls still execute inline until their callers are migrated.
- Step-move note: queued `apply_move` also accepts `move_value` for forward movement. In that mode it computes the path, queues one `resolve_lap_reward` per crossed start tile, then queues `resolve_arrival` when landing resolution is requested. LAP rewards and tile effects therefore have separate Redis checkpoints.
- Adapter note: `_build_standard_move_action()` / `_enqueue_standard_move_action()` convert resolved normal movement into a queued `apply_move` action. The adapter now mirrors obstacle slowdown, encounter boost, and zone-chain follow-up movement for parity tests. The legacy `_take_turn()` path still stores turn-end work in `pending_turn_completion` for compatibility; module-runner sessions immediately attach that envelope to the active `TurnFrame`'s `TurnEndSnapshotModule`.
...
- Turn-completion note: legacy `run_next_transition()` can still drain `pending_turn_completion` for compatibility. In the module runtime, turn completion is owned by `TurnEndSnapshotModule` inside the current `TurnFrame`; orphan `pending_turn_completion` checkpoints and sequence-owned turn-end adapters are invalid.
...
- 2026-05-03 target-judicator sync: module runtime no longer resolves mark-target character abilities directly inside `CharacterStartModule`. Character start now yields to `TargetJudicatorModule`; queued marks materialize for `PendingMarkResolutionModule`, while 자객's immediate reveal/skip effect is inserted as `ImmediateMarkerTransferModule`. This keeps Redis resume boundaries aligned with the active turn module and prevents trick continuation from replaying character-start targeting.
