...
from policy.asset.spec import ArenaPolicyAsset, DEFAULT_ARENA_CHARACTER_LINEUP, HeuristicPolicyAsset, MultiAgentBattleAsset
...
class PolicyFactory:
    ...
    @staticmethod
    def normalize_multi_agent_battle_asset(asset: MultiAgentBattleAsset | None = None) -> MultiAgentBattleAsset:
        source = asset or MultiAgentBattleAsset()
        player_specs = {int(pid): str(spec) for pid, spec in dict(source.player_specs or {}).items()}
        default_spec = str(source.default_spec or "gpt:v3_gpt")
        for pid in range(1, 5):
            player_specs.setdefault(pid, default_spec)
        return MultiAgentBattleAsset(
            player_specs=player_specs,
            default_spec=default_spec,
        )

    @staticmethod
    def create_multi_agent_dispatcher(asset: MultiAgentBattleAsset | None = None):
        from multi_agent.agent_loader import make_agent
        from multi_agent.dispatcher import MultiAgentDispatcher

        normalized_asset = PolicyFactory.normalize_multi_agent_battle_asset(asset)
        agents = {
            pid: make_agent(spec)
            for pid, spec in normalized_asset.player_specs.items()
        }
